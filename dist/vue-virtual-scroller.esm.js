...
var Scroller = {
  ...
  computed: {
    heights: function heights() {
      if (this.itemHeight === null) {
        ...
        var cacheHeights = this.cacheHeights || {};
        var accumulator = 0;
        var current = void 0;
  ...
  methods: {
    ...
    getAccumulator: function getAccumulator(index) {
      var accumulator = index > 0 ? this.heights[index - 1].accumulator : 0;
      return accumulator;
    },
    scrollToItem: function scrollToItem(index, eltra = 0) {
      this.$nextTick(function () {
        var scrollTop = void 0;
        if (this.itemHeight === null) {
          scrollTop = index > 0 ? this.heights[index - 1].accumulator - eltra : 0;
        } else {
    ...
    itemsLimitError: function itemsLimitError() {
      ...
      setTimeout(function () {
        console.log('It seems the scroller element isn\'t scrolling, so it tries to render all the items at once.', 'Scroller:', _this2.$el);
...
function __vue_normalize__(template, style, script, scope, functional, moduleIdentifier, createInjector, createInjectorSSR) {
  ...
  component.__file = "/home/atom/vue-virtual-scroller/src/components/RecycleScroller.vue";
...
var script$1 = {
    ...
    itemsWithHeight: function itemsWithHeight() {
      ...
      for (var i = 0; i < items.length; i++) {
        ...
        result.push({
          item: item,
          id: id,
          height: heights[id] || cacheHeights[id] || 0
        });
        // console.log(cacheHeights[id] + ', ' + new Date().getTime())
      }
      return result;
    },
  ...
  methods: {
    ...
    scrollToItem: function scrollToItem(index, eltra = 0) {
      var scroller = this.$refs.scroller;
      if (scroller) scroller.scrollToItem(index, eltra);
    },
    scrollToBottom: function scrollToBottom(force) {
      var scroller = this.$refs.scroller;
      if (scroller) scroller.scrollToBottom(force);
    }
  }
};
...
function __vue_normalize__$1(template, style, script, scope, functional, moduleIdentifier, createInjector, createInjectorSSR) {
  ...
  // For security concerns, we use only base name in production mode.
  component.__file = "/home/atom/vue-virtual-scroller/src/components/DynamicScroller.vue";
...
var script$2 = {
  ...
  mounted: function mounted() {
    ...
    if (this.vscrollData.active) {
      this.updateSize();
      this.$nextTick(function () {
        _this2.mountedCallback(_this2.item, _this2.index);
      });
    }
  },
    ...
    computeSize: function computeSize(id) {
      ...
      this.$nextTick(function () {
        if (_this4.id === id) {
          ...
          if (size.height && _this4.height !== size.height) {
            _this4.$set(_this4.vscrollData.heights, _this4.id, size.height);
            // 更新缓存元素高度
            _this4.cacheHeights = Object.assign(_this4.cacheHeights || {}, _this4.vscrollData.heights);
            if (_this4.emitResize) _this4.$emit('resize', _this4.id);
          }
        }
        _this4.$_pendingSizeUpdate = false;
      });
    }
  },
...
function __vue_normalize__$2(template, style, script, scope, functional, moduleIdentifier, createInjector, createInjectorSSR) {
  ...
  // For security concerns, we use only base name in production mode.
  component.__file = "/home/atom/vue-virtual-scroller/src/components/DynamicScrollerItem.vue";
